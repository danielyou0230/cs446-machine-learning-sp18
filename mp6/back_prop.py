...
class Neural_Network():
    ...
    def d_loss_o(self, gt, o):
        ...
        outputs:
            derivative of the L2 loss with respect to the network's
    ...
    def error_at_layer2(self, d_o, o):
        ...
        inputs:
            d_o: derivative of the loss with respect to the network output
            (batchSize, outputSize)
            o: the network output (batchSize, outputSize)
        returns:
            the derivative of the loss with respect to the output of the
            second fully connected layer (batchSize, outputSize).
        '''
        return (d_o * self.sigmoidPrime(o))

    def error_at_layer1(self, delta_k, W, b):
        '''
        computes the derivative of the loss with respect to layer1
        (Written section, Part e).
        inputs:
            delta_k: derivative of the loss with respect to the output of
            the second fully connected layer (batchSize, outputSize).
            W: the weights of the second fully connected layer
            (hiddenSize, outputSize).
            b: the input to the second fully connected layer
            (batchSize, hiddenSize).
        returns:
            the derivative of the loss with respect to the output of the second
            fully connected layer (batchSize, hiddenSize).
        '''
        return (delta_k.dot(W.T)) * self.sigmoidPrime(b)

    def derivative_of_w(self, b, delta_k):
        '''
        computes the derivative of the loss with respect to W
        (Written section, Part c).
        inputs:
            b: the input to the second fully connected layer
            (batchSize, hiddenSize).
            delta_k: the derivative of the loss with respect to the output of
            the second fully connected layer's output (batchSize, outputSize).
        returns:
            the derivative of loss with respect to W (hiddenSize ,outputSize).
        '''
        return (1 / batchSize)*(b.T.dot(delta_k))

    def derivative_of_u(self, X, delta_j):
        '''
        computes the derivative of the loss with respect to U
        (Written section, Part f).
        inputs:
            X: the input to the network (batchSize, inputSize).
            delta_j: the derivative of the loss with respect to the output of
            the first fully connected layer's output (batchSize, hiddenSize).
        returns:
            the derivative of loss with respect to U (inputSize, hiddenSize).
        '''
        return (1 / batchSize) * (X.T.dot(delta_j))

    def derivative_of_e(self, delta_j):
        '''
        computes the derivative of the loss with respect to e
        (Writtensection, Part g).
        inputs:
            delta_j: the derivative of the loss with respect to the output of
            the first fully connected layer's output (batchSize, hiddenSize).
        returns:
            the derivative of loss with respect to e (hiddenSize).
        '''
        return np.mean(delta_j, axis=0)

    def derivative_of_f(self, delta_k):
        '''
        computes the derivative of the loss with respect to f
        (Written section, Part d).
        inputs:
            delta_k: the derivative of the loss with respect to the output of
            the second fully connected layer's output (batchSize, outputSize).
        returns:
            the derivative of loss with respect to f (outputSize).
        '''
        return np.mean(delta_k, axis=0)
    ...
    def backward(self, X, gt, o):
        ...
        self.grad_U = self.derivative_of_u(X, delta_j)

        # 7. Compute the derivative of e (Written section, Part g).
        # Call: derivative_of_e
        self.grad_e = self.derivative_of_e(delta_j)
