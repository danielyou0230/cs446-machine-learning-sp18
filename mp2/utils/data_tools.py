...
def preprocess_data(dataset,
    ...
    # end of debug
    # print (dataset['581'])
    # x = np.array([dataset[entry] for entry in dataset])
    x = list()
    y = list()
    for entry in dataset:
        current_entry = list()
        # print (dataset[entry])
        for idx, element in enumerate(dataset[entry]):
            if idx == 0:
                continue
            if idx != len(dataset[entry]) - 1:
                try:
                    if squared_features:
                        current_entry.append(int(element) ** 2)
                    else:
                        current_entry.append(int(element))
                except ValueError:
                    # current_entry.append(one_hot_bldg_type(element))
                    attribute = one_hot_bldg_type(element)
            else:
                y.append(int(element))
        # add one-hot code to the entry
        current_entry += attribute
        # Add parsed entry to x
        x.append(current_entry)
    #
    x = np.array(x)
...
def one_hot_bldg_type(bldg_type):
    ...
    # One-hot Encoding
    ret = [0] * len(type_to_id)
    ret[type_to_id[bldg_type]] = 1
    # print ("{0}: {1}".format(type_to_id[bldg_type], ret))
    return ret
