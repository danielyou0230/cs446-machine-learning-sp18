...
def get_action_index(readout_t, epsilon, t):
    ...
    """
    if t < OBSERVE:
        action_index = np.random.choice(len(readout_t), 1)
    else:
        tmp = np.argmax(readout_t)
        others = np.random.choice(len(readout_t), 1)
        action_index = np.random.choice([tmp, others], 1, p=[1 - epsilon, epsilon])
...
def run_selected_action(a_t, s_t, game_state):
    ...
    s_t1 = np.concatenate([s_t[:,:,1:], x_t1[:,:, np.newaxis]], axis=-1)
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        ...
        q = r_batch[i] + GAMMA * np.max(readout_j1_batch)
        target_q_batch.append(r_batch[i] if terminal_batch[i] else q)
