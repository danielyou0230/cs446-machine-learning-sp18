...
class VariationalAutoencoder(object):
    ...
    def _sample_z(self, z_mean, z_log_var):
        ...
        epsilon = dist.sample(tf.shape(z_mean))

        # Reparametrization on z
    ...
    def _encoder(self, x):
        ...
        ####### Implementation Here ######
        # Encoder
        layers = fully_connected(x, num_outputs=100)
        layers = fully_connected(layers, num_outputs=50, activation_fn=tf.nn.softplus)
        # output
        z_mean = fully_connected(layers, num_outputs=self._nlatent, activation_fn=None)
        z_log_var = fully_connected(layers, num_outputs=self._nlatent, activation_fn=None)
    ...
    def _decoder(self, z):
        ...
        ####### Implementation Here ######
        layers = fully_connected(z, num_outputs=50, activation_fn=tf.nn.relu)
        layers = fully_connected(layers, num_outputs=100, activation_fn=tf.nn.softplus)
        f = fully_connected(layers, num_outputs=self._ndims, activation_fn=tf.sigmoid)
    ...
    def _latent_loss(self, z_mean, z_log_var):
        ...
        p = tf.distributions.Normal(loc=np.zeros(self._nlatent, dtype=np.float32),
                                    scale=np.ones(self._nlatent, dtype=np.float32))
        q = tf.distributions.Normal(loc=z_mean, scale=sigma)
        latent_loss = tf.reduce_sum(tf.distributions.kl_divergence(q, p))
        """
        latent_loss = 0.5 * tf.reduce_mean(tf.exp(z_log_var) + tf.square(z_mean) - z_log_var - 1)
    ...
    def loss(self, f, x_gt, z_mean, z_log_var):
        ...
        ####### Implementation Here ######
        # Calculate losses
        latent_loss = self._latent_loss(z_mean, z_log_var)
