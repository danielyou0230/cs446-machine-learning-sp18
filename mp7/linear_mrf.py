...
class LinearMRF(object):
    ...
    def get_unary_features(self, img):
        ...
        #######################
        possible_val = [0, 1]
        features = [np.equal(img, itr) for itr in possible_val]
    ...
    def get_pairwise_features(self):
        ...
        # define possible values that y's would take
        possible_val = [0, 1]
        for m in possible_val:
            for n in possible_val:
                features.append([m == n] * n_pairs)
        #
        return np.array(features, dtype=np.float32).T
    ...
    def calculate_unary_potentials(self, unary_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        potentials = self.unary_weight * \
                     tf.convert_to_tensor(unary_features, dtype=tf.float32)
        return potentials
    ...
    def calculate_pairwise_potentials(self, pairwise_features):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        potentials = self.pairwise_weight * \
                     tf.convert_to_tensor(pairwise_features, dtype=tf.float32)
        return potentials
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        unary_beliefs = [tf.cast(itr, dtype=tf.float32) for itr in unary_beliefs]
        pair_beliefs = [tf.cast(itr, dtype=tf.float32) for itr in pair_beliefs]
        unary_potentials = [tf.cast(itr, dtype=tf.float32) for itr in unary_potentials]
        pairwise_potentials = tf.cast(pairwise_potentials, dtype=tf.float32)
        # Ground Truth
        # use ground truth as beliefs of the input feature of 
        # both unary and pairwise beliefs
        true_belief = tf.convert_to_tensor(img_features, dtype=tf.float32)
        true_pw_belief = self.get_pairwise_beliefs(img_features)
        true_pw_belief = tf.convert_to_tensor(true_pw_belief, dtype=tf.float32)

        obj = 0.
        # unary
        for unary_belief, unary_potential in zip(unary_beliefs, unary_potentials):
            obj += tf.reduce_sum(unary_belief * unary_potential)
            obj -= tf.reduce_sum(true_belief * unary_potential)

        # pairwise
        for pair_belief in pair_beliefs:
            obj += tf.reduce_sum(pair_belief * pairwise_potentials)
            obj -= tf.reduce_sum(true_pw_belief * pairwise_potentials)
        return obj
    ...
    def run_greedy_inference(self, unary_beliefs, unary_pots, pairwise_pots,
        ...
        for unary_belief, unary_pot in zip(unary_beliefs, unary_pots):
            ...
            while not converged:
                ...
                itr += 1
                print(" - Inference_itr: {0}".format(itr))
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        new_unary_beliefs = np.zeros(unary_beliefs.shape)
        n_nodes = unary_beliefs.shape[0]
        # 1. Determine a random order of nodes
        ...
        #   a. Calculate the scores (using calculate_local_score) for that
        #      node having assignment 0 and 1
            s_0 = self.calculate_local_score(itr, 0, unary_beliefs, unary_pots, pairwise_pots)
            s_1 = self.calculate_local_score(itr, 1, unary_beliefs, unary_pots, pairwise_pots)
        #   b. Set the belief for the assignment having the larger score to 1
        #      and that for the assignment having the smaller score to 0
            new_unary_beliefs[itr, 0 if s_0 > s_1 else 1] = 1
        return new_unary_beliefs
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        #######################
        pairwise_score = 0.
        for itr in self.neighbors[node]:
            # for each pair, generate the tuple
            pair = (itr, node) if itr < node else (node, itr)
            # convert to pairwise index
            pairwise_i = self.pair_inds[pair]
            # combination index
            neighbor_i = unary_beliefs[itr].argmax()
            comb_index = neighbor_i * 2 + assignment \
                         if itr < node else \
                         assignment * 2 + neighbor_i
            # accumulate the potential based on unary beliefs
            pairwise_score += pairwise_potentials[pairwise_i, comb_index]

        score = unary_potentials[node, assignment] + pairwise_score
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        change = np.average(np.equal(new_unary_beliefs, old_unary_beliefs))
        return True if change >= 1 - convergence_margin else False
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        for assignment (y_m, y_n) = 1 iff the unary belief for 
        node m with assignment y_m is 1 and the unary belief for 
        node n with assignment y_n is 1.
        ...
        #######################
        possible_val = [0, 1]
        n_possible = len(possible_val)
        for idx, itr in enumerate(self.pairs):
            # unpack the node indices
            m, n = itr
            # convert the indices with 1 in both nodes to pairwise index
            # 00 -> 0
            # 01 -> 1
            # 10 -> 2
            # 11 -> 3
            entry = unary_beliefs[m,:].argmax() * n_possible + unary_beliefs[n,:].argmax()
            result[idx, entry] = 1
